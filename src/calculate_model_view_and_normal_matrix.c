...
void calculate_model_view_and_normal_matrix(mat4 mv[restrict 1], mat3 n[restrict 1], object_t const o[1], vec3 cp) {
	...
	AT(mv, 0, 1) = o->scale.x * s; AT(mv, 1, 1) =  o->scale.y * c; AT(mv, 3, 1) = o->position.y - cp.y;
